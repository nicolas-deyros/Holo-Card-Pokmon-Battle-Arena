...
export const TeamSelection: React.FC<TeamSelectionProps> = ({ initialPokemonName, onTeamConfirm, onBack }) => {
    ...
    const addPokemonToTeam = useCallback(async (name: string, targetIndex?: number) => {
    ...
    const handleRemovePokemon = (id: number) => {
        setTeam(currentTeam => currentTeam.map(p => p?.id === id ? null : p).filter(Boolean) as Pokemon[]);
    };

    const handleDragStart = (e: DragEvent, pokemon: Pokemon, sourceType: 'team' | 'popular', index?: number) => {
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            ...
            <p className="text-slate-300 text-center mb-8">Choose {TEAM_SIZE} Pokémon. Drag and drop to build your team or reorder.</p>
            ...
            {!teamIsFull && (
                <div className="w-full max-w-2xl mx-auto">
                    ...
                    <h3 className="text-xl font-bold text-center mt-8 mb-4">Or click/drag a popular one to a slot:</h3>
                    {popularPokemon.length === 0 ? <LoadingSpinner /> : (
                        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                             {popularPokemon.map(p => {
                                ...
                                return (
                                    ...
                                    >
                                        <img src={p.sprites.front_default} alt={p.name} className="mx-auto h-16 w-16 pointer-events-none" />
                                        <p className="capitalize font-semibold text-xs mt-1 pointer-events-none">{p.name}</p>
                                    </div>
                                );
                             })}
                        </div>
