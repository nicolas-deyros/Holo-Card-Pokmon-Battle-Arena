...
const TypeBadge: React.FC<{ typeName: string }> = ({ typeName }) => (
    <span className={`px-2 py-0.5 text-xs font-bold uppercase rounded-full ${TYPE_COLORS[typeName] || 'bg-gray-500'} bg-opacity-80 backdrop-blur-sm`}>
        {typeName}
    </span>
);

const RemoveButton: React.FC<{ onClick: () => void }> = ({ onClick }) => (
    <button onClick={onClick} className="absolute top-2 right-2 z-20 bg-red-600 hover:bg-red-500 rounded-full w-8 h-8 flex items-center justify-center text-white font-bold transition-transform transform hover:scale-110" aria-label="Remove Pokémon">
...
export const PokemonCard: React.FC<PokemonCardProps> = ({ pokemon, isActive, isPlayer, onAttack, onRemove, isFainted, isHit, isStatic, displayMode = 'full' }) => {
                ...
                <div className="holo-card-header">
                    ...
                    {displayMode !== 'hand' && (
                      <div className="holo-card-hp-info">
                          ...
                          <span className="holo-card-hp-value"> {pokemon.hp}/{pokemon.maxHp}</span>
