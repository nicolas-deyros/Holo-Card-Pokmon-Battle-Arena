import type { Pokemon, Attack, PokeApiPokemon, PokeApiMove, PokeApiType, PokeApiSpecies, PokeApiEvolutionChain, EvolutionLink } from '../types';
...
export async function getAllPokemonNames(): Promise<{ name: string }[]> {
    const data = await cachedFetch<{ results: { name: string }[] }>(`${POKEAPI_BASE_URL}/pokemon?limit=1302`);
    return data.results;
}
...
export async function getTypeRelations(typeName: string): Promise<{ weaknesses: string[], resistances: string[] }> {
    ...
    return {
        ...
        resistances: typeData.damage_relations.half_damage_from.map(t => t.name).concat(typeData.damage_relations.no_damage_from.map(t => t.name)),
    };
}

export async function getPokemonDetails(nameOrId: string | number): Promise<Pokemon | null> {
    try {
        const pokemonData = await cachedFetch<PokeApiPokemon>(`${POKEAPI_BASE_URL}/pokemon/${nameOrId}`);
        ...
        const allTypeNames = (await cachedFetch<{results: {name: string}[]}>(`${POKEAPI_BASE_URL}/type`)).results.map(r => r.name).filter(n => n !== 'unknown' && n !== 'shadow');
        ...
        for (const typeInfo of pokemonData.types) {
            const typeData = await cachedFetch<PokeApiType>(`${POKEAPI_BASE_URL}/type/${typeInfo.type.name}`);
            typeData.damage_relations.double_damage_from.forEach(t => { if(typeEffectiveness[t.name] !== undefined) typeEffectiveness[t.name] *= 2 });
            typeData.damage_relations.half_damage_from.forEach(t => { if(typeEffectiveness[t.name] !== undefined) typeEffectiveness[t.name] *= 0.5 });
            typeData.damage_relations.no_damage_from.forEach(t => { if(typeEffectiveness[t.name] !== undefined) typeEffectiveness[t.name] = 0 });
        }

        const allWeaknesses = Object.entries(typeEffectiveness).filter(([,mult]) => mult >= 2).map(([type]) => type);
        const allResistances = Object.entries(typeEffectiveness).filter(([,mult]) => mult < 1).map(([type]) => type);
...
export async function getEvolutionLine(pokemonName: string): Promise<Pokemon[] | null> {
    try {
        const speciesData = await cachedFetch<PokeApiSpecies>(`${POKEAPI_BASE_URL}/pokemon-species/${pokemonName}`);
        const evolutionChainData = await cachedFetch<PokeApiEvolutionChain>(speciesData.evolution_chain.url);
