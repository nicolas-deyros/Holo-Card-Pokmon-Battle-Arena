...
type GameAction =
  ...
  | { type: 'PROCESS_TURN'; payload: { damage: number; log: string; isSuperEffective: boolean; isResisted: boolean } }
...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    case 'START_SUCCESS':
      return {
        ...initialState,
        player: { hand: action.payload.playerTeam.slice(1), activePokemon: action.payload.playerTeam[0] },
        computer: { hand: action.payload.computerTeam.slice(1), activePokemon: action.payload.computerTeam[0] },
        gameStatus: 'playing',
        log: [`Battle between ${action.payload.playerTeam[0].name} and ${action.payload.computerTeam[0].name} starts!`],
      };
    
    case 'ATTACK': {
      const attackerName = state.turn === 'player' ? state.player.activePokemon?.name : state.computer.activePokemon?.name;
      const attackerIdentifier = state.turn === 'player' ? 'Player' : 'Computer';
      return { ...state, isProcessingTurn: true, pendingAttack: action.payload.attack, log: [...state.log, `${attackerIdentifier}'s ${attackerName} used ${action.payload.attack.name}!` ] };
    }
    ...
    case 'PROCESS_TURN': {
        ...
        if (state.turn === 'player') {
            ...
            return {
                ...state,
                computer: { ...state.computer, activePokemon: { ...state.computer.activePokemon!, hp: newHp } },
                log: newLog,
            };
    ...
    case 'FAINT_SWITCH': {
        if(state.turn === 'player') { // Computer fainted
            const logMessage = `Computer's ${state.computer.activePokemon?.name} fainted!`;
            if (action.payload.nextComputerPokemon) {
                 return { 
                     ...state, 
                     computer: { ...state.computer, activePokemon: action.payload.nextComputerPokemon, hand: state.computer.hand.filter(p => p.id !== action.payload.nextComputerPokemon!.id) }, 
                     log: [...state.log, logMessage, `Computer sends out ${action.payload.nextComputerPokemon.name}!`],
                     isProcessingTurn: false,
                     pendingAttack: undefined,
                    };
            }
            return { ...state, log: [...state.log, logMessage], isProcessingTurn: false, pendingAttack: undefined, };
        } else { // Player fainted
             return { 
                ...state, 
                turn: 'player',
    ...
    case 'GAME_OVER':
      return { ...state, turn: 'gameover', gameStatus: action.payload.winner === 'player' ? 'player_win' : 'computer_win', log: [...state.log, `All of ${action.payload.winner === 'player' ? 'the Computer' : 'the Player'}'s Pokémon have fainted!`] };
...
async function calculateDamage(attack: Attack, attacker: Pokemon, defender: Pokemon) {
    let damage = attack.power || 20;
    let multiplier = 1.0;
...
export const useGameLogic = (playerTeam: Pokemon[]) => {
    ...
    if (gameState.gameStatus === 'setup' && playerTeam.length > 0) {
      const fetchComputerTeam = async () => {
        try {
          ...
          const computerTeam = (await Promise.all(computerPromises)).filter((p): p is Pokemon => p !== null);

          if (isSubscribed && playerTeam.length === TEAM_SIZE && computerTeam.length === TEAM_SIZE) {
  ...
  useEffect(() => {
    ...
    const processTurn = async () => {
      if (gameState.isProcessingTurn && gameState.pendingAttack) {
        ...
        const { damage, isSuperEffective, isResisted } = await calculateDamage(pendingAttack, attacker, defender);

        if (!isSubscribed) return;
        dispatch({ type: 'PROCESS_TURN', payload: { damage, log: '', isSuperEffective, isResisted } });
        ...
        turnTimer = window.setTimeout(() => {
          ...
          const defenderAfterDamage = attackerIsPlayer ? currentGameState.computer.activePokemon : currentGameState.player.activePokemon;
  ...
  useEffect(() => {
    ...
    if (gameState.turn === 'computer' && !gameState.isProcessingTurn && gameState.gameStatus === 'playing') {
  ...
  const handleAttack = useCallback((attack: Attack) => {
    if (gameState.turn !== 'player' || gameState.isProcessingTurn || gameState.player.activePokemon?.hp === 0) return;
    dispatch({ type: 'ATTACK', payload: { attack } });
  }, [gameState.turn, gameState.isProcessingTurn, gameState.player.activePokemon?.hp]);

  const handleSwitchPokemon = useCallback((pokemonId: number) => {
